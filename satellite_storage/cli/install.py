...
Five phases (modeled on the primary's ``cli/commands/install.py`` UX):
...
Registration (step 5) is the terminal confirmation — the primary's
response carries the assigned satellite record, which the wizard prints.
We do NOT re-fetch ``/fleet/inventory`` from here: that endpoint is
loopback-only (operator-side), so a satellite calling it over Tailscale
always gets 403. The operator verifies from the primary with
``molebie-ai extend list``.
...
def install_command(
    ...
    total_steps = 3 if foreground else 5
    ...
    print_info("Verify from the primary with: [bold]molebie-ai extend list[/bold]")
